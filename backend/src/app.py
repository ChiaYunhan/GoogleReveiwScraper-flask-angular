            ...
            jsonify(
                {
                    ...
                    "message": "Reviews scraped. Download as CSV or Upload to AWS s3.",
                }
            ),
            200,
        )
...
def upload_s3():
    data = request.json
    location_name = data.get("location_name", "unknown_location")

    # Generate a default CSV name with a timestamp and location name
    default_name = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{location_name}.csv"
    csv_name = request.args.get("csv_name") or default_name
    csv_data = session.get("csv_data")

    if not csv_data:
        return jsonify({"error": "No CSV data available. Please scrape first."})
